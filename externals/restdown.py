...
from externals.markdown2 import Markdown, UnicodeWithAttrs, _regex_from_encoded_pattern
...
def restdown_path(path, brand_dir=None, copy_brand_media_to=None, defines=None):
    markdown = path
    metadata = {}
    metadata["markdown2extras"] = "wiki-tables, cuddled-lists"
    ...
    for key in ["apisections", "markdown2extras", "markdown2linkpatternsfile"]:
        ...
        if key in metadata:
            ...
            reader = csv.reader(f)
            try:
                row = [cell.strip() for cell in next(reader)]
